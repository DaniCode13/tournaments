...
export default {
    ...
    data() {
        return {
            title_tournament: this.title,
            ids_elements_deleted:[],
            participants_data: {
                teams: [],
                results: []
            }
    ...
    methods: {
        elements_deleted:function(data){
           var arr_new=[]
           data.forEach(team => {
                team.forEach(participant=>{
                    arr_new.push(participant)
                })
            });
            var arr_original = []
            this.participants_data.teams.forEach(team => {
                team.forEach(participant=>{
                    arr_original.push(participant)
                })
            });
            var compare = arr_original.filter((item)=>{
               return !arr_new.includes({id:item.id})
        ...
        format_data_tournament: function () {
            ...
            for (let index = 0; index < this.data.length; index+=2) {
               items.push([this.data[index], this.data[index+1]])
            }
        ...
        edit_fn: function (container, data, doneCb) {
            ...
            input.blur(function () {
                ...
                doneCb({img: img, name: '',id:id });
                } else {
                    doneCb({img: img, name: inputValue,id:id });
                }
                console.log('edit')
                console.log(data)
            });
        ...
        add_order_to_participants: function () {
            ...
            this.elements_deleted(data_reformatted);
        ...
        save_tournament: function (data) {
            ...
            axios.patch('/t/update/'+this.id, params)
                .then((res) => {
